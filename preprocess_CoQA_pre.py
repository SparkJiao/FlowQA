...
def proc_train(ith, article):
    ...
    for j, (question, answers) in enumerate(zip(article['questions'], article['answers'])):
        ...
        if j > 0:
            q_text = article['questions'][j - 1]['input_text'] + " // " + article['answers'][j - 1]['input_text'] + " // " + q_text
        if j > 1:
            q_text = article['questions'][j - 2]['input_text'] + " // " + article['answers'][j - 1]['input_text'] + " // " + q_text
        if j > 2:
            q_text = article['questions'][j - 3]['input_text'] + " // " + article['answers'][j - 1]['input_text'] + " // " + q_text
...
with open('CoQA/train_pre_data.msgpack', 'wb') as f:
...
with open('CoQA/dev_pre_data.msgpack', 'wb') as f:
    msgpack.dump(result, f)
