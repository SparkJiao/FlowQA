...
from general_utils_a import flatten_json, free_text_to_span, normalize_text, build_embedding, load_glove_vocab, pre_proc, get_context_span, \
...
def proc_train(ith, article):
    ...
    for j, (question, answers) in enumerate(zip(article['questions'], article['answers'])):
        ...
        if j > 0:
            a_text = article['answers'][j - 1]['input_text']
        else:
            a_text = ''

        rows.append((ith, q_text, a_text, answer, answer_start, answer_end, rationale, rationale_start, rationale_end, answer_choice))
    return rows, context


train, train_context = flatten_json(trn_file, proc_train)
train = pd.DataFrame(train, columns=['context_idx', 'q_text', 'a_text', 'answer', 'answer_start', 'answer_end', 'rationale', 'rationale_start',
...
trQ_iter = (pre_proc(q) for q in train.q_text)
trA_iter = (pre_proc(a) for a in train.a_text)
...
trC_string = [' '.join(doc) for doc in trC_tokens]
trQ_string = [' '.join(doc) for doc in trQ_tokens]
trA_string = [' '.join(doc) for doc in trA_tokens]
trQ_tokens = [["<S>"] + doc + ["</S>"] for doc in trQ_tokens]
trQ_ids = [[2] + qsent + [3] for qsent in trQ_ids]
print(trQ_ids[:10])
trA_tokens = [["<S>"] + doc + ["</S>"] for doc in trA_tokens]
trA_ids = [[2] + qsent + [3] for qsent in trA_ids]
print(trA_ids[:10])
# tags
...
result = {
    'question_string': trQ_string,
    'answer_string': trA_string,
    'context_string': trC_string,
    ...
    'q_text': train.q_text.tolist(),
    'a_text': train.a_text.tolist(),
...
dev = pd.DataFrame(dev, columns=['context_idx', 'q_text', 'a_text', 'answer', 'answer_start', 'answer_end', 'rationale', 'rationale_start',
                                 'rationale_end', 'answer_choice'])
...
devQ_iter = (pre_proc(q) for q in dev.q_text)
devA_iter = (pre_proc(a) for a in dev.a_text)
...
devC_string = [' '.join(doc) for doc in devC_tokens]
devQ_string = [' '.join(doc) for doc in devQ_tokens]
devA_string = [' '.join(doc) for doc in devA_tokens]
...
devA_ids = [[2] + qsent + [3] for qsent in devA_ids]
print(devA_ids[:10])
# tags
...
result = {
    'question_string': devQ_string,
    'answer_string': devA_string,
    'context_string': devC_string,
    ...
    'q_text': dev.q_text.tolist(),
    'a_text': dev.a_text.tolist(),
