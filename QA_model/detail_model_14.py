...
class FlowQA(nn.Module):
    ...
    def __init__(self, opt, embedding=None, padding_idx=0):
        ...
        if opt['do_prealign']:
            ...
            self.pre_align = layers_kai.BiDAF(embedding_dim, opt['hidden_size'], opt['prealign_hidden'], similarity_attention=True)
            doc_input_size += embedding_dim + 2 * opt['hidden_size']
        if opt['no_em']:
    ...
    def forward(self, x1, x1_c, x1_f, x1_pos, x1_ner, x1_mask, x2_full, x2_c, x2_full_mask):
        ...
        if self.opt['do_prealign']:
            x1_atten = self.pre_align(x1_emb_expand, x2_emb, x1_mask, x2_mask)
