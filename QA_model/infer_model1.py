...
class InferQA(nn.Module):
    """Network for the InferQA Module."""

    def __init__(self, opt, embedding=None, padding_idx=0):
        super(InferQA, self).__init__()

        print('Use InferQA model.')
        ...
        if opt['do_prealign']:
            self.emb_multihead_selfattn = layers.MultiHeadSelfAttn(embedding_dim, head_num=5)
            self.emb_infer = layers.InferModule(embedding_dim)
            self.init_emb_feature = Parameter(torch.zeros(1, embedding_dim), requires_grad=True)

            self.pre_align = layers.GetAttentionHiddens(embedding_dim, opt['prealign_hidden'], similarity_attention=True)
            doc_input_size += 2 * embedding_dim
        if opt['no_em']:
        ...
        # ans_hidden_size = que_hidden_size
        ...
        if opt['self_attention_opt'] > 0:
            ...
            self.high_lvl_crnn, doc_hidden_size = layers.RNN_from_opt(doc_hidden_size * 2 + flow_size, opt['hidden_size'], opt, num_layers=1,
                                                                      concat_rnn=False)
            ...
            self.high_lvl_crnn, doc_hidden_size = layers.RNN_from_opt(doc_hidden_size + flow_size, opt['hidden_size'], opt, num_layers=1,
                                                                      concat_rnn=False)

        print('Before answer span finding, hidden size are', doc_hidden_size, que_hidden_size)

        # Question merging
        self.hidden_multihead_selfattn = layers.MultiHeadSelfAttn(que_hidden_size, head_num=5)
        self.hidden_infer = layers.InferModule(que_hidden_size)
        self.init_hidden_feature = Parameter(torch.zeros(1, que_hidden_size), requires_grad=True)
        ...
        @ Kai
        """
        self.flow_att = layers_kai.ConversationAtt(doc_hidden_size, opt['hidden_size'])

        # Store config
        self.opt = opt

    def forward(self, x1, x1_c, x1_f, x1_pos, x1_ner, x1_mask, x2_full, x2_c, x2_full_mask, x3_full, x3_c, x3_full_mask):
        """Inputs:
        ...
        if self.opt['use_elmo'] and self.opt['elmo_batch_size'] > self.opt['batch_size']:  # precomputing ELMo is used
            ...
            layer_activations = [t[x1.size(0) * self.precomputed_cnt: x1.size(0) * (self.precomputed_cnt + 1), :, :] for t in
        ...
        drnn_input_list, qrnn_input_list, arnn_input_list = [], [], []
        ...
        if self.opt['CoVe_opt'] > 0:
            ...
            arnn_input_list.append(x3_cove_mid)
        ...
        if self.opt['use_elmo']:
            ...
            x3_elmo = self.elmo(x3_c)['elmo_representations'][
                0]  # torch.zeros(x2_emb.size(0), x2_emb.size(1), 1024, dtype=x2_emb.dtype, layout=x2_emb.layout, device=x2_emb.device)
            # Dropout on contexualized embeddings
            if self.opt['dropout_emb'] > 0:
            ...
            arnn_input_list.append(x3_elmo)
        ...
        x3_input = torch.cat(arnn_input_list, dim=2)
        ...
        if self.opt['do_prealign']:
            x2_emb_feature = self.emb_multihead_selfattn(x2_emb, x2_mask)  # batch * embedding_size
            x3_emb_feature = self.emb_multihead_selfattn(x3_emb, x3_mask)  # batch * embedding_size
            pre_x2_emb_feature = torch.cat([self.init_emb_feature, x2_emb_feature[:-1]], dim=0)  # only work for batch=1
            slots = torch.stack([x3_emb_feature, pre_x2_emb_feature], dim=1)
            que_feature, _ = self.emb_infer(slots, x2_emb_feature)  # batch * embedding_size

            x1_atten = self.pre_align(x1_emb_expand, x2_emb, x2_mask)
            x1_input = torch.cat([x1_input, x1_atten, que_feature.unsqueeze(1).repeat(1, x1_input.size(1), 1)], dim=2)
        ...
        _, ans_abstr_ls = self.question_rnn(x3_input, x3_mask, return_list=True, additional_x=x3_cove_high)

        # Final question layer
        question_hiddens = self.high_lvl_qrnn(torch.cat(que_abstr_ls, 2), x2_mask)
        que_abstr_ls += [question_hiddens]
        answer_hiddens = self.high_lvl_qrnn(torch.cat(ans_abstr_ls, 2), x3_mask)
        ans_abstr_ls += [answer_hiddens]

        # Get question_avg_hidden
        q_merge_hiddens = self.hidden_multihead_selfattn(question_hiddens, x2_mask)  # batch * hidden_size
        a_merge_hiddens = self.hidden_multihead_selfattn(answer_hiddens, x3_mask)  # batch * hidden_size
        pre_q_merge_hiddens = torch.cat([self.init_hidden_feature, q_merge_hiddens[:-1]], dim=0)  # only work for batch=1
        slots = torch.stack([a_merge_hiddens, pre_q_merge_hiddens], dim=1)
        question_avg_hidden, _ = self.hidden_infer(slots, q_merge_hiddens)  # batch * hidden_size
        ...
        # x1_att = torch.cat([x1_att, question_avg_hidden.unsqueeze(1).repeat(1, x1_att.size(1), 1)], 2)

        if self.opt['self_attention_opt'] > 0:
            highlvl_self_attn_hiddens = self.highlvl_self_att(x1_att,
                                                              x1_att,
                                                              x1_mask, x3=doc_hiddens, drop_diagonal=True)
        ...
        doc_abstr_ls += [doc_hiddens]
        """ Kai """
        doc_hiddens = self.flow_att(doc_hiddens.reshape(x1_full.size(0), x1_full.size(1), x1_full.size(2), -1),
                                    x1_mask.reshape(x1_full.size(0), x1_full.size(1), x1_full.size(2)))
