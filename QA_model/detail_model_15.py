...
class FlowQA(nn.Module):
    ...
    def __init__(self, opt, embedding=None, padding_idx=0):
        ...
        """ @Spark Jiao """
        self.iterator1 = layers_kai.BiDAF(opt['hidden_size'] * 2, opt['hidden_size'] * 2, opt['hidden_size'], similarity_attention=True)
        self.iterator2 = layers_kai.BiDAF(opt['hidden_size'] * 2, opt['hidden_size'] * 2, opt['hidden_size'], similarity_attention=True)
        self.iterator3 = layers_kai.BiDAF(opt['hidden_size'] * 2, opt['hidden_size'] * 2, opt['hidden_size'], similarity_attention=True)
    ...
    def forward(self, x1, x1_c, x1_f, x1_pos, x1_ner, x1_mask, x2_full, x2_c, x2_full_mask):
        ...
        # @Spark Jiao
        def DocAtt(c, q, c_mask, q_mask, iterator):
            batch_size, max_qa_pair, context_length = x1_full.size()
            residual_layer = iterator(c, q, c_mask, q_mask).view(batch_size, max_qa_pair, context_length, -1)
            out = residual_layer.new_zeros(residual_layer.size())
            for i in range(1, max_qa_pair, 1):
                out[:, i, :, :] = residual_layer[:, i - 1, :, :]
            out = out.reshape(batch_size * max_qa_pair, context_length, -1)
            return out

        # Encode question with RNN
        question_hiddens, que_abstr_ls = self.question_rnn(x2_input, x2_mask, return_list=True, additional_x=x2_cove_high)
        ...
        doc_hiddens_flow = DocAtt(doc_hiddens, question_hiddens, x1_mask, x2_mask, self.iterator1)
        ...
        doc_hiddens_flow = DocAtt(doc_hiddens, question_hiddens, x1_mask, x2_mask, self.iterator2)
        doc_abstr_ls.append(doc_hiddens)
        ...
        doc_hiddens_flow = DocAtt(doc_hiddens, question_hiddens, x1_mask, x2_mask, self.iterator3)
