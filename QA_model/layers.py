...
class DocumentAttn(nn.Module):
    def __init__(self, input_size, attention_hidden_size, num_layers=1):
        super(DocumentAttn, self).__init__()
        self.scoring = AttentionScore(input_size, attention_hidden_size)
        self.num_layers = num_layers

    def forward(self, c1, c2, c_mask):
        c = c2
        assert c1.size(1) == c2.size(1)
        c_len = c1.size(1)
        c_mask = c_mask.unsqueeze(1).expand(-1, c_len, -1)
        diag_mask = torch.diag(c.data.new(c_len).zero_() + 1).byte().unsqueeze(0).expand_as(c_mask)

        for i in range(self.num_layers):
            scores = self.scoring(c, c1)
            scores.data.masked_fill_(c_mask.data, -float('inf'))
            ...
            # c2 to c1 attention distribution
