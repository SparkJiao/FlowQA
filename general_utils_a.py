    ...
    def __iter__(self):
        ...
        for batch_i in range((self.context_num + self.batch_size - 1) // self.batch_size):
            ...
            for i, q_seq in enumerate(question_batch):
                for j, id in enumerate(q_seq):
                    ...
                    for prv_ctx in range(0, self.dialog_ctx):
                        if j > prv_ctx:
                            prv_id = id - prv_ctx - 1
                            prv_ans_st, prv_ans_end, prv_rat_st, prv_rat_end, prv_ans_choice = qa_data[prv_id][3], qa_data[prv_id][4], \
                                                                                               qa_data[prv_id][5], qa_data[prv_id][6], \
                                                                                               qa_data[prv_id][7]
            ...
            for i, q_seq in enumerate(question_batch):
                ...
                for j, id in enumerate(q_seq):
                    answer_s[i, j], answer_e[i, j], rationale_s[i, j], rationale_e[i, j], answer_c[i, j] = qa_data[id][3], qa_data[id][4], \
                                                                                                           qa_data[id][5], qa_data[id][6], \
                                                                                                           qa_data[id][7]
                    overall_mask[i, j] = 1
